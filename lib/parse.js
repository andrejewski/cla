...
function expandArgAlias(scopes, arg) {
  for(let i = 0; i < scopes.length; i++) {
    ...
    const alias = scope.aliases.find((alias) => {
      return alias.name === arg && expandsWithinScope(alias.value, scopes[0]);
    });
...
function expandArgFlag(scopes, arg) {
  ...
  for(let i = 0; i < scopes.length; i++) {
    ...
    const flag = scope.flags.find((flag) => {
      return flag.name === arg && expandsWithinScope(flag.value, scopes[0]);
    });
...
function expandsWithinScope(expansion, scope) {
  const commands = expansion.filter((expr) => expr.type === 'command');
  const unknown = commands.find((command) => {
    return !scope.commands.find((cmd) => command.name === cmd.name);
  });
  return !unknown;
}
